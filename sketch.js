...
//****** NEWLEY ADDED****
var eris_speechStage = 1;
var eris_speechStageB = 1;
var eris_speechCounter1 = 0;
var eris_speechCounter2 = 0;
var eris_speechCounter3 = 0;
...
function draw() {
  ...
  //ECHO
    ...
    if (echo_stage == 1) {
      ...
      if (echo_keyCounter > 5) {
        echo_guide.guideDisplay(1, "", "Echo, signing on.", "", "Observing HUMAN#" + human + " typing,", " while MACHINE counts inputs.", "");
    ...
    // letters
      ...
      if (echo_stage == 2) {
        echo_guide.guideDisplay(2, "", "Input threshold reached.", "", "MACHINE has generated poem for", "HUMAN#" + human + " to reveal.", "");
        echo_guide.guideMove();
      ...
      if (echo_stage == 3) {
        ...
        if (echoComplete == false) {
          if (echo_keyInput.length <= 5) {
            echo_guide.guideDisplay(2, "", "Input threshold reached.", "", "MACHINE has generated poem for", "HUMAN#" + human + " to reveal.", "");
          } else if (echo_keyInput.length > 5 && echo_keyInput.length <= 25) {
            echo_guide.guideDisplay(3, "", "", "Observing HUMAN#" + human + " offer letters", "for MACHINE to place.", "", "");
          } else if (echo_keyInput.length > 25) {
            echo_guide.guideDisplay(4, "", "", "MACHINE waiting to place final", "letters from HUMAN#" + human, "", "");
          }
          echo_guide.guideMove();
        } else if (echoComplete == true) {
          echo_guide.guideDisplay(5, "", "Poem complete.", "Experiment concluded.", "", "Echo, signing off.", "");
          echo_guide.guideMove();
          echo_guide.guidehover();
  ...
  //ERIS/ANERIS
    ...
    console.log(eris_stage, eris_speechStage)

    if (eris_stage == 1) {
      if (mouseX - pmouseX > width / 3.5 || mouseX - pmouseX < -(width / 3.5) ||
        mouseY - pmouseY > height / 3.5 || mouseY - pmouseY < -(height / 3.5)) {
          eris_stage = 2;
        //  eris_speechStage = 1;
        }
      ...
      if (eris_speechStage == 1 || eris_speechStage == 2) {
        eris_guide.guideDisplay(1, "", "Eris, signing on.", "", "HAHA Aneris can't get in type while", "HUMAN#" + human + " is moving their mouse", "");
      } else if (eris_speechStage == 3 || eris_speechStage == 4) {
        eris_guide.guideDisplay(2, "I, Eris, am watching MACHINE glitch", "through poetry arrays again...", "", "HUMAN#" + human + " seems perplexed.", "", "haha Aneris is still stuck outside");
      }
      ...
      if (eris_DarkCounter >= 95) {
        ...
        if (eris_speechStage == 1) {
          aneris_guide.guideDisplay(1, "", "Aneris, signing on.", "", "Eris?", "Where are you?", "");
          eris_speechCounter1 = 0
        } else if (eris_speechStage == 2 || eris_speechStage == 3) {
          aneris_guide.guideDisplay(2, "", "", "Let me in, Eris.", "I don’t have time for your games.", "", "");
          eris_speechCounter2++;
          eris_speechCounter3 = 0;
        } else if (eris_speechStage == 4) {
          aneris_guide.guideDisplay(3, "", "", "HUMAN#" + human + " be a dear,", "try larger & faster mouse movements.", "", "");
        }
        aneris_guide.guideMove();

      } else {
        if (eris_speechStage == 1 || eris_speechStage == 2) {
          eris_speechCounter1++
          eris_speechCounter2 = 0;
        } else if (eris_speechStage == 3) {
          eris_speechCounter3++;
        }
      }

      if (eris_speechCounter1 == 100) eris_speechStage = 2;
      if (eris_speechCounter2 == 50) eris_speechStage = 3;
      if (eris_speechCounter3 == 100) eris_speechStage = 4;
      ...
      if (eris_speechStageB == 1) {
        aneris_guide.guideDisplay(4, "Thank you HUMAN#" + human + ".", "To edit the machine poem, begin by", "moving your mouse up & down", "to highlight a line.", "", "Click for further instruction.");
        eris_guide.guideDisplay(3, "", "What! Aneris?", "", "How'd you get in here?!", "Hey! don't ignore me!", "");
      } else if (eris_speechStageB == 2) {
        aneris_guide.guideDisplay(5, "Then, move your left & right", "to change the word.", "", "When you are satisfied with", "all of your choices,", "click anywhere to proceed.");
        eris_guide.guideDisplay(4, "I, Eris, am woeful!", "", "HUMAN#" + human + " has mastered", "the MACHINE monster, and is", "now toggling through", "machine-poetry arrays.", "");
      }
      aneris_guide.guideMove();
      eris_guide.guideMove();
      ...
      eris_guide.guideDisplay(5, "A-HA! MACHINE has shuffled", "all of HUMAN#" + human + "’s hard work.", "I am delighted!", "", "Eris, gleefully signing off.", "P.S. Aneris has been banished again");
      eris_guide.guideMove();
      ...
      if (eris_DarkCounter > 95) {
        ...
        aneris_guide.guideDisplay(6, "Don't mind my sister.", "MACHINE has re-stacked the poem,", "as it should read.", "", "Your choices have been cherished.", "Aneris, signing off.");
        aneris_guide.guideMove();
      }
  ...
  //chiron
    ...
    background(0, 0, 0);
    gridbackground(15, 0.1, 100, 0, height, 150, -90, 200);
    for (var i = 0; i < stars.length / 2; i++) {
      stars[i].display(0);
    }
    ...
    if (chiron_stage == 1) {
      ...
      chiron_guide.guideDisplay(2, "MACHINE reveals that HUMAN#" + human, "has discovered a constellation.", "", "I await patiently for", "HUMAN#" + human + " to exhibit some", "autonomous decision-making.");
      chiron_guide.guideMove();
      ...
      if (chiron_mouseA.length < 5) {
        ...
        chiron_guide.guideDisplay(4, "", "", "HUMAN#" + human + " continues their search", "for the remaining lost words.", "", "");
      }
      chiron_guide.guideMove();
...
function mousePressed() {
  if (slide == -1) {
    ...
    if (eris_stage == 2) {
      eris_DarkCounter = 0;
      eris_speechStageB++;
      if (eris_speechStageB > 2) eris_stage = 3;
    }
